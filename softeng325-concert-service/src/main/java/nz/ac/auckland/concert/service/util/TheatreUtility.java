...
public class TheatreUtility {
	...
	public static Set<Seat> getSpecificAvailableSeats(int numberOfSeats, List<Seat> openSeats) {
	...
	public static Set<Seat> findAvailableSeats(int numberOfSeats, List<Seat> seats){
		Set<Seat> availableSeats = new HashSet<Seat>();
		int startIndex = 0;
		...
		while (numberOfSeats > 0) {
			if (startIndex > seats.size() - 1) {
				startIndex = 0;
			}
			availableSeats.add(seats.get(startIndex));
			startIndex++;
			numberOfSeats--;
		}
		return availableSeats;
